...
import { chatAPI, usersAPI } from '@/api/index.js'
  ...
  if (Array.isArray(groups.value)) {
    groups.value.forEach(g => {
      list.push({
        ...
        _time: formatTime(g.last_message_at),
    ...
    privates.value.forEach(p => {
      ...
      list.push({
        _key: "p-" + (other.user_id || p.message_id),
        _type: "private",
        _name: other.nickname || "用户",
        _avatar: other.avatar_url || "",
        _initial: (other.nickname || "?")[0],
        _avatarBg: {},
        _preview: p.content || "暂无消息",
        _time: formatTime(p.created_at),
        _unread: (other.user_id && !p.is_read) ? 1 : 0,
        _targetId: other.user_id
      })
    })
  }
...
function onMsgClick(item) {
  if (item._type === 'group') {
    router.push('/messages/' + item._targetId)
  } else {
