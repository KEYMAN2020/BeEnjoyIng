...
def list_notifications():
    ...
    total = execute_query_one(f"SELECT COUNT(*) AS total FROM notifications n WHERE {where}", params)["total"]

    offset = (page - 1) * per_page
    rows = execute_query(
        f"SELECT * FROM notifications n WHERE {where} ORDER BY n.created_at DESC LIMIT %s OFFSET %s",
    ...
    return success({
        "notifications": [{
            ...
            "content": r.get("content", ""), "ref_type": r.get("ref_type"),
            "ref_id": r.get("ref_id"), "is_read": bool(r["is_read"]),
        ...
        "unread_count": execute_query_one(
            "SELECT COUNT(*) AS cnt FROM notifications WHERE user_id = %s AND is_read = 0", (user_id,),
        )["cnt"],
    })
...
def mark_notification_read(notification_id):
    ...
    execute_update("UPDATE notifications SET is_read = 1, read_at = NOW() WHERE id = %s", (notification_id,))
